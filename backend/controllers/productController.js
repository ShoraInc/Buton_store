...
const productController = {
  ...
  getProducts: async (req, res) => {
    try {
      ...
      console.log(
        `✅ Найдено товаров: ${count}, на странице: ${products.length}`
      );
      ...
      if (imagesFromBody) {
        try {
          ...
          if (Array.isArray(existingImages)) {
            ...
            console.log(
              "📸 Добавлены существующие изображения:",
              existingImages
            );
          }
        } catch (error) {
  ...
  updateProduct: async (req, res) => {
    try {
      ...
      if (updateData.name && !updateData.name.trim()) {
        return res
          .status(400)
          .json({ message: "Название товара не может быть пустым" });
      }
      ...
      // ИСПРАВЛЕННАЯ ЧАСТЬ: Преобразование числовых значений
      ["price", "discountPrice", "inStock"].forEach((field) => {
        if (updateData[field] !== undefined) {
          ...
          if (
            ...
            // Для discountPrice устанавливаем null, для остальных - 0
            updateData[field] = field === "discountPrice" ? null : 0;
          } else {
            // Преобразуем в число
            const numValue = parseFloat(updateData[field]);
            updateData[field] = isNaN(numValue)
              ? field === "discountPrice"
                ? null
                : 0
              : numValue;
      ...
      if (!updated) {
        return res
          .status(404)
          .json({ message: "Товар не найден или не обновлен" });
      }
  ...
  deleteProduct: async (req, res) => {
    try {
      ...
      console.log(
        "✅ Товар успешно удален (мягкое удаление):",
  ...
  getCategories: async (req, res) => {
    try {
      ...
      const allCategories = [
        ...new Set([...predefinedCategories, ...categoryList]),
      ];
